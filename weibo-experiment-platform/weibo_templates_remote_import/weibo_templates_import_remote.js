// 微博评论模板导入脚本（80 条：low 40 + high 40）
// 用法: mongosh "mongodb://<user>:<pass>@<host>:27017/<dbname>" weibo_templates_import_remote.js
// 幂等：按 (post_group, content) upsert；不在本清单内的旧模板会被标记 is_active:false
// 集合名: weibo_comment_templates（与 weibo-experiment-platform 的 cn() 前缀一致）
...
const coll = db.getCollection('weibo_comment_templates');
let upserted = 0, unchanged = 0;
for (const t of tpls) {
  const r = coll.findOneAndUpdate(
    { post_group: t.post_group, content: t.content },
    { $set: { sort_order: t.sort_order, is_active: true }, $setOnInsert: { created_at: new Date().toISOString() } },
    { upsert: true, returnDocument: "after" }
  );
  if (r.created_at) { upserted++; } else { unchanged++; }
}
// 禁用不在清单内的旧模板
const keep = new Set(tpls.map((t) => t.content));
const disabled = coll.updateMany(
  { is_active: true, content: { $nin: [...keep] } },
  { $set: { is_active: false } }
).modifiedCount;
print(`导入完成: 新增/更新 ${upserted + unchanged} 条 (新增 ${upserted}, 已存在 ${unchanged}), 禁用旧模板 ${disabled} 条`);
print(`当前 active 模板数: ${coll.countDocuments({ is_active: true })}`);
